...
var ManageCoursesPage = React.createClass({
	...
	statics: {
		willTransitionFrom: function(transition, component) {
			if (component.state.dirty && !confirm('Are you leaving without saving?')) {
				transition.abort();
			}
		}
	},
	...
	courseFormIsValid: function() {
		...
		if (this.state.course.author.length < 3) {
			this.state.errors.author = 'Last name must be at least 3 characters.';
			formIsValid = false;
		}

		if (this.state.course.category.length < 3) {
			this.state.errors.category = 'First name must be at least 3 characters.';
			formIsValid = false;
		}

		if (this.state.course.length.length < 3) {
			this.state.errors.length = 'Last name must be at least 3 characters.';
			formIsValid = false;
		}
